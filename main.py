...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpocmxwcG5rbmZqeGh3aGZzZHhkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTY1NjY3NjIsImV4cCI6MjA3MjE0Mjc2Mn0.EVrzx09YwDglwFUCjS3hKbrg2Wdy1hjSPV1gWxnN_yU"  # use service_role key for server writes
...
def get_vehicle_stats(vehicle_id: str):
    response = supabase.table("sensor_data").select("*").eq("vehicle_id", vehicle_id).execute()
    if response.status_code >= 400:
        raise HTTPException(status_code=response.status_code, detail=str(response.data))
